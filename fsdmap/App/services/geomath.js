/**
 * geographical calculations based on functions from
 ...
 * specified by latitude/longitude (in numeric degrees)
 * 
 * from: Haversine formula - R. W. Sinnott, "Virtues of the Haversine", Sky and
 * Telescope, vol 68, no 2, 1984 http://www.census.gov/cgi-bin/geo/gisfaq?Q5.1
 * 
 * example usage from form: result.value =
 * LatLon.distHaversine(lat1.value.parseDeg(), long1.value.parseDeg(),
 * lat2.value.parseDeg(), long2.value.parseDeg()); where lat1, long1, lat2,
 * long2, and result are form fields
 */
...
// extend String object with method for parsing degrees or lat/long values to
// numeric degrees
//
// this is very flexible on formats, allowing signed decimal degrees, or
// deg-min-sec suffixed by
// compass direction (NSEW). A variety of separators are accepted (eg 3º 37'
// 09"W) or fixed-width
// format without separators (eg 0033709W). Seconds and minutes may be omitted.
// (Minimal validation
// is done).

String.prototype.parseDeg = function () {
    if (!isNaN(this)) return Number(this);                 // signed decimal
    // degrees without
    // NSEW

    var degLL = this.replace(/^-/, '').replace(/[NSEW]/i, '');  // strip off any
    // sign or compass
    // dir'n
    var dms = degLL.split(/[^0-9.,]+/);                     // split out
    // separate d/m/s
    for (var i in dms) if (dms[i] == '') dms.splice(i, 1);    // remove empty
    // elements (see
    // note below)
    switch (dms.length) {                                  // convert to decimal degrees...
        ...
        case 1:                                              // decimal or non-separated dddmmss
            if (/[NS]/i.test(this)) degLL = '0' + degLL;       // - normalise N/S
            // to 3-digit
            // degrees
    ...
    if (/^-/.test(this) || /[WS]/i.test(this)) deg = -deg; // take '-', west
    // and south as -ve
    return deg;
}
...
Number.prototype.toDMS = function () {  // convert numeric degrees to deg/min/sec
    var d = Math.abs(this);  // (unsigned result ready for appending compass
    // dir'n)
...
Number.prototype.toLat = function () {  // convert numeric degrees to deg/min/sec latitude
    return this.toDMS().slice(1) + (this < 0 ? 'S' : 'N');  // knock off initial '0'
    // for lat!
}
...
Number.prototype.toPrecision = function (fig) {  // override toPrecision method with one which displays
    if (this == 0) return 0;                      // trailing zeros in place of
